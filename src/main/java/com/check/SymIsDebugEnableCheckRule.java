...
public class SymIsDebugEnableCheckRule extends BaseTreeVisitor implements JavaFileScanner {
...
//	}
	...
	private static SymIsDebugEnableCheckRule getInstance() {
		return new SymIsDebugEnableCheckRule();
	}
	...
	public void visitTryStatement(TryStatementTree tree) {
		...
		for (StatementTree st : list) {
			...
			if (st.is(Tree.Kind.IF_STATEMENT)) {
				// calling statement
				SymIsDebugEnableCheckRule.getInstance().processingStatement(st);
			}
			if (st.is(Tree.Kind.EXPRESSION_STATEMENT)) {
				ExpressionStatementTree est = (ExpressionStatementTree) st;
				if (est.expression() instanceof MethodInvocationTree) {
					// calling expression-1
					String name = SymIsDebugEnableCheckRule.getInstance().processingFirstExpression(est);
					if ("debug".equals(name)) {
						if (!statementsStack.isEmpty()) {
							...
							if (lastStmt.is(Tree.Kind.IF_STATEMENT)) {
								...
								String value = SymIsDebugEnableCheckRule.getInstance()
	 ...
	 * Symbol.TypeSymbol symbol = tree.symbol(); Type type = symbol.type();
	 * IdentifierTree simpleName = tree.simpleName();
	 * System.out.println("----"+tree.simpleName().getClass().getPackage());
	 * System.out.println("simpleName-"+simpleName);
	 * System.out.println("type.isSubtypeOf(\"java.util.Comparator\")"+tree.
	 * simpleName().getClass().getPackage());
	 * 
	 ...
	 * //
	 * 
	 * if (vt.type().equals("Logger")) { flag = true; System.out.println("flag:" +
	 * flag); } System.out.println("type--" + vt.type());
	 * System.out.println("variable"); } // System.out.println("---> member" +
					...
					if (est.expression() instanceof MethodInvocationTree) {
						...
						if (est1.methodSelect() instanceof MemberSelectExpressionTree) {
							...
							if ("debug".equals(name)) {
								...
								System.out.println(
										"REPORT ISSUE FOR Method: isDebugEnable missing before logger statement");
							}
						}
					}
				}
	...
	public void visitIfStatement(IfStatementTree tree) {
		    //BinaryExpressionTreeImpl binaryExpressionTreeImpl = (BinaryExpressionTreeImpl) tree.condition();
		String name2 = null;
		String nameB=null;
		
		//
		if(tree.is(Tree.Kind.METHOD_INVOCATION)) {
		MethodInvocationTree conditionTree = (MethodInvocationTree) tree.condition();
		MemberSelectExpressionTree imset = (MemberSelectExpressionTree) conditionTree.methodSelect();
		IdentifierTree id = (IdentifierTree) imset.identifier();
		String name = id.name();
		System.out.println("NAME----"+name);
		
		BlockTree blockTree2 = (BlockTree) tree.thenStatement();
		List<StatementTree> st2 = blockTree2.body();
		for(StatementTree stt2:st2)
		{
			if(stt2.is(Tree.Kind.EXPRESSION_STATEMENT))
			{
			ExpressionStatementTree expressionStatementTree = (ExpressionStatementTree)stt2;
			MethodInvocationTree mit = (MethodInvocationTree) expressionStatementTree.expression();
			MemberSelectExpressionTree mset2 = (MemberSelectExpressionTree) mit.methodSelect();
			IdentifierTree idt = (IdentifierTree)	mset2.identifier();
			 name2= idt.name();
			System.out.println("NAME2"+name2);
		}
		}//if close
		//if("isDebugEnabled".equals(name) && binaryExpressionTreeImpl.is(Tree.Kind.CONDITIONAL_AND))
	
			
		if (name.equals("isDebugEnabled")&& !name2.equals("debug")) {
			context.reportIssue(this, tree,
					" LOG RULE EXPRESSION: missing before logger statement");
			System.out.println("REPORT ISSUE EXPRESSION: <---> missing isDebug() in if condition> isDebugEnable missing before logger statement");
		}
		/*
		 * if (tree.expression() instanceof MethodInvocationTree) { MethodInvocationTree
		 * est1 = (MethodInvocationTree) tree.expression(); if (est1.methodSelect()
		 * instanceof MemberSelectExpressionTree) { MemberSelectExpressionTree mset =
		 * (MemberSelectExpressionTree) est1.methodSelect(); IdentifierTree it =
		 * (IdentifierTree) mset.identifier(); String name = it.name(); if
		 * ("debug".equals(name)) { context.reportIssue(this, tree,
		 * " LOG RULE EXPRESSION: isDebugEnable missing missing before logger statement"
		 * ); System.out.
		 * println("REPORT ISSUE EXPRESSION: isDebugEnable missing before logger statement"
		 * ); } } }
		...
		else 
		{
			...
			BinaryExpressionTree bet = (BinaryExpressionTree) tree.condition();
			
			//TODO ckeck left/right operand
			//if(bet.is(Tree.Kind.LEFT_SHIFT))
			methodInvocationTree = (MethodInvocationTree) bet.leftOperand();
			
			/*
			 * else methodInvocationTree = (MethodInvocationTree) bet.rightOperand();
			 */
			...
			String nameA = identifierTree.name();//isDebugEnabled
			
			BlockTree blockTree = (BlockTree) tree.thenStatement();
			List<StatementTree> st = blockTree.body();
			for(StatementTree stt:st)
			{
				if(stt.is(Tree.Kind.EXPRESSION_STATEMENT))
				{
				ExpressionStatementTree expressionStatementTree = (ExpressionStatementTree)stt;
				MethodInvocationTree mit = (MethodInvocationTree) expressionStatementTree.expression();
				MemberSelectExpressionTree mset2 = (MemberSelectExpressionTree) mit.methodSelect();
				IdentifierTree idt = (IdentifierTree)	mset2.identifier();
				 nameB= idt.name();//info
			...
			if (nameA.equals("isDebugEnabled")&& !nameB.equals("debug")) {
				context.reportIssue(this, tree,
						" LOG RULE EXPRESSION: missing before logger statement");
				System.out.println("REPORT ISSUE EXPRESSION: <---> missing isDebug() in if condition> isDebugEnable missing before logger statement");
			}
